...
object SesionesEstudio : IntIdTable("sesiones_estudio") {
    val usuarioId = integer("usuario_id").references(Usuarios.id)
    val materiaId = integer("materia_id").references(Materias.id)
    val fecha = date("fecha")
    // Se guardan segundos para conservar precisión durante pruebas y sesiones cortas.
    val duracionSegundos = integer("duracion_segundos")
    val metaMinutos = integer("meta_minutos").default(0)
    val createdAt = timestamp("created_at").clientDefault { java.time.Instant.now() }
}

object AsesoriasCalificaciones : IntIdTable("asesorias_calificaciones") {
    val asesoriaId = integer("asesoria_id").references(Asesorias.id).uniqueIndex()
    val asesorId = integer("asesor_id").references(Usuarios.id)
    val alumnoId = integer("alumno_id").references(Usuarios.id)
...
object Notificaciones : IntIdTable("notificaciones") {
    ...
    val titulo = varchar("titulo", 200)
    val contenido = text("contenido")
    val tipo = varchar("tipo", 40).default("GENERAL")
    val referenciaId = integer("referencia_id").nullable()
    // Clave opcional para recordatorios: evita insertar el mismo aviso más de una vez.
    val clave = varchar("clave", 180).nullable().uniqueIndex()
