...
fun Routing.peersRoutes() {
    ...
    post("/peers/solicitudes") {
        ...
        val id = transaction {
            val nuevaSolicitudId = PeersSolicitudes.insertAndGetId {
                it[autorId] = userId
            ...
            val autor = Usuarios.selectAll().where { Usuarios.id eq userId }.firstOrNull()
            val nombreAutor = autor?.get(Usuarios.nombre) ?: "Un alumno"
            val cuatrimestreAutor = autor?.get(Usuarios.cuatrimestre) ?: 1
            Usuarios.selectAll()
                .where { Usuarios.id neq userId }
                .filter { row ->
                    val rol = row[Usuarios.rol]
                    rol == "ASESOR" || rol == "ADMIN" ||
                        (rol == "ALUMNO" && row[Usuarios.cuatrimestre] >= cuatrimestreAutor)
                }
    ...
    post("/peers/solicitudes/{id}/cerrar") {
        ...
        val updated = transaction {
            val solicitud = PeersSolicitudes.selectAll()
                .where { PeersSolicitudes.id eq solicitudId }
                .firstOrNull()
            val filas = if (rol == "ASESOR" || rol == "ADMIN") {
            ...
            val autorId = solicitud?.get(PeersSolicitudes.autorId)
            if (filas > 0 && autorId != null && autorId != userId) {
                crearNotificacion(
                    usuarioId = autorId,
                    titulo = "Tu pregunta fue cerrada",
                    contenido = "Un asesor cerró la solicitud: ${solicitud[PeersSolicitudes.titulo]}",
                    tipo = "PEERS_SOLICITUD",
